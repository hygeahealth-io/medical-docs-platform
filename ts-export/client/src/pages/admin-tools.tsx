      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {vitalsSettings.vitalSigns.map((vital: string, index: number) => (
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {qualitySettings.pdfComparisonFields.map((field: string, index: number) => (
                <div key={index} className="flex items-center space-x-2 p-2 border rounded">
                  ...
                  <Button 
                    onClick={() => {
                      const newFields = qualitySettings.pdfComparisonFields.filter((_: any, i: number) => i !== index);
                      setQualitySettings({...qualitySettings, pdfComparisonFields: newFields});
                    }}
                    size="sm" 
                    variant="ghost"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {qualitySettings.automationFields.map((field: string, index: number) => (
                <div key={index} className="flex items-center space-x-2 p-2 border rounded">
                  <Input 
                    value={field}
                    onChange={(e) => {
                  ...
                  <Button 
                    onClick={() => {
                      const newFields = qualitySettings.automationFields.filter((_: any, i: number) => i !== index);
                      setQualitySettings({...qualitySettings, automationFields: newFields});
                    }}
                    size="sm" 
                    variant="ghost"
...
function CustomKeysSettings({ settings, onSave, isLoading }: any) {
  ...
  const updateKeyBinding = (id: number, field: string, value: string) => {
    setKeyBindings(keyBindings.map((kb: any) => 
      kb.id === id ? { ...kb, [field]: value } : kb
    ));
  };

  const removeKeyBinding = (id: number) => {
    setKeyBindings(keyBindings.filter((kb: any) => kb.id !== id));
  };
